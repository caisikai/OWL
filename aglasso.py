...
MAX_ITER_SPARSE_GROUP = 1000
MAX_ITER_ADAPTIVE = 20
...
def sparse_group_lasso(X, y, alpha, rho, groups, max_iter=MAX_ITER_SPARSE_GROUP, rtol=1e-6,
                verbose=False):
    """
...
def adaptive_sparse_group_lasso(X, y, alpha, rho, groups,max_iter_adaptive=MAX_ITER_ADAPTIVE,
        rtol=1e-6,verbose=False):
    group_labels = [np.where(groups == i)[0] for i in np.unique(groups)]
    weights=np.ones(len(group_labels))
    X_w=np.zeros_like(X)
    for iter in range(max_iter_adaptive):
        for i,group in enumerate(group_labels):
            X_w[:,group]=X[:,group]/weights[i]
        coefs = sparse_group_lasso(X, y, alpha, rho, groups, rtol=rtol, verbose=verbose)
        for i,group in enumerate(group_labels):
            coefs[group]=coefs[group]/weights[i]
            weights[i]=1/(2. * np.sqrt(np.sum(np.abs(coefs[group])))+np.finfo(float).eps)
    return coefs
...
if __name__ == '__main__':
    ...
    coefs = adaptive_sparse_group_lasso(X, y, alpha, 0., groups, verbose=True)
